<script setup>
  ...
  const selectedSound = ref(sounds.value[0]); // Selecciona el primer sonido por defecto
  const isPlaying = ref(false);
  const audio = ref(new Audio(selectedSound.value.file)); // Inicializar el audio con el primer sonido
  ...
  onMounted(() => {
    ...
    audio.value.volume = volume.value; // Inicializar el volumen
  });
